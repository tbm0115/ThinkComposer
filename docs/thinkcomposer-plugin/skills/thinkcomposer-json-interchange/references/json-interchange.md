...
Saved `.tcom` and `.tdom` packages can also contain AI-readable sidecar snapshots under `/Interchange/`. These embedded JSON files are generated by the same exporters documented here, plus a `/Interchange/manifest.json` described in `docs/container-readable-snapshots.md`. They are inspection/context snapshots only; native load still reads `/Composition.bin` or `/Domain.bin`.
